...
document.querySelector(".search_result_div");
...
const CurrTvshow = async (id, title) => {
...
const CurrEpisode = async (series_id, currSea, currEpiso) => {
  let episohtml;
  const res = await fetch(
    `https://api.themoviedb.org/3/tv/${series_id}/season/${currSea}/episode/${currEpiso}?api_key=6b2dec73b6697866a50cdaef60ccffcb`
  );
  const data = await res.json();
...
const releasedEpisode = function (curre) {
  return `<div><img class="episode_image" src="https://image.tmdb.org/t/p/w500/${curre.still_path}" alt="${
    curre.name
  }">
    <span class="runtime">${timeCon(curre.runtime)}</span>
<div href="#${curre.episode_number}" class="playsvg_container tvOpen1"><img class="playsvg" src="./resources/play-circle-fill.svg"
        alt=""></div>
</div>
<div class="episode_detail">
<h2 class="episode_title"> <span class="episodenumm">${
    curre.episode_number
  }.</span> ${curre.name} - NAME ITO 6  ${curre.series_id}</h2>
<p class="episode_description">${curre.overview}</p>
</div>
<div id="${curre.episode_number}" class="tvOpen mfp-hide">
  <iframe src="https://flicky.host/embed/tv/?id=${curre.id}/${curre.season_number}/${curre.episode_number}&server=OYO" style="position:relative;border:none;" allowfullscreen></iframe>
</div>`;
};

const unreleasedEpisodev = function (curre) {
  return `<a class="unreleased_link" href=" https://flicky.host/embed/tv?id=${fetcid}&s=${
    curre.season_number
